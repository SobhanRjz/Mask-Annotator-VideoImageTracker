...
class TrackingService:
    ...
    def start(self,mid,aids,start,end,step=5,replace=True):
        ...
        if not(0<=start<m['frame_count'] and 0<=end<m['frame_count']):raise ValueError('Frame out of range')
    ...
    def _run(self,j):
        try:
            ...
            for index,(aid,lid,masks) in enumerate(outputs,start=1):
                if not masks:continue
                created.extend(annotation_service.bulk_save(j.media_id,lid,masks,'auto','sam2-%s-%s'%(j.id[:8],index)))
